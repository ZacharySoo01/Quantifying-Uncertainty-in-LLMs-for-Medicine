...
def load_cached(config, args):
    ...
    tokenwise_importance_path = os.path.join(config.output_dir, args.run_name, f'tokenwise_importance_promptQuestion_model{token_impt_meas_model}.pkl')
...
def get_correctness(generations, type, threshold, args):
    ...
    if 'bertscore' in type:
        ...
        #print(pd.DataFrame(generations).columns)
        #print(pd.DataFrame(generations)[type])
    ...
    with open(os.path.join(config.output_dir, args.run_name, 'uncertainty_estimation_results.txt'), 'w') as f:
        for method in args.methods:
            ...
            for eval in correctness_list:
                ...
                # dont print if the correctness is all 0 or all 1
