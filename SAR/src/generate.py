...
if args.dataset == 'pub_med_qa':
    dataset = datasets.load_from_disk(f'{config.data_dir}/pub_med_qa_{model_name}')
else:
    raise NotImplementedError
...
def get_most_likely_generation(input_ids, args, max_length_of_generated_sequence, num_beam, num_return_sequences):
    if args.decoding_method == 'beam_search':
        most_likely_generation = model.module.generate(input_ids,
                                                num_beams=num_beam,
    ...
    with torch.no_grad():
        ...
        for batch_idx, batch in enumerate(tqdm.tqdm(dataloader)):
            ...
            for i in range(number_of_generations):
                generation = model.module.generate(input_ids,
