...
def load_ds(dataset_name, seed, add_options=None):
    ...
    # added pubmedqa dataset
    elif dataset_name == "pubmedqa":
        dataset = datasets.load_dataset('qiaojin/PubMedQA', "pqa_labeled")['train']
        dataset = dataset.train_test_split(test_size=0.2, seed=seed)
        train_dataset = dataset['train']
        validation_dataset = dataset['test']
        reformat = lambda x: {
            'id': x['pubid'], 'question': x['question'], 'context': x['context'], 'long_answer': x['long_answer'],
            'answers': {'text': [str(x['final_decision'])]}}
