...
def get_parser(stages=['generate', 'compute']):
    ...
    if 'generate' in stages:
        ...
        parser.add_argument(
            ...
            choices=['trivia_qa', 'squad', 'bioasq', 'nq', 'svamp', 'pubmedqa'],
            help="Dataset to use")
        parser.add_argument(
...
def init_model(args):
    mn = args.model_name
    if 'llama' in mn.lower() or 'falcon' in mn or 'mistral' in mn.lower() or 'gemma' in mn.lower() or 'opt' in mn.lower() or 'phi-2' in mn.lower() or 'gpt-neo' in mn.lower():
